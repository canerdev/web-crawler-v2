...
class DenoisingMLP(nn.Module):
    ...
    Intermediate layer sizes are calculated dynamically relative to item_dim:
...
class ConditionalDiffusion(nn.Module):
    ...
    Fully dynamic based on embed_dim, accommodating different dataset 
    dimension requirements without structural changes.
    ...
    def __init__(
        ...
        self.model = DenoisingMLP(
            item_dim=item_dim,
            cond_dim=cond_dim,
            time_dim=item_dim,  # time_dim always matches item_dim
            dropout=dropout
        )
    ...
    def forward(self, target_item_emb, condition):
        ...
        """
        # Normalize to maintain consistency with the cosine similarity space used in inference
        ...
        # Classifier-Free Guidance: randomly drop the condition during training
        if self.p_uncond > 0:
    ...
    def _reverse_diffusion(self, x, condition, w):
        ...
        Args:
            ...
            w         : CFG weight (higher = more condition-guided)
    ...
    def sample(self, condition, target_domain_embs, watched_ids=None, w=2.0, k=10):
        ...
        Args:
            condition          : (B, D) — L2-normalized c_ud
            target_domain_embs : (N, D) — all target item embeddings (index 0 = PAD)
            watched_ids        : (B, S) optional — already-interacted indices to suppress
            w                  : CFG weight
