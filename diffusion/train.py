...
def train():
    ...
    print(
        f"Mode: {'Triple-Stream (GNN + Source + Target)' if use_source_stream else 'Ablation (GNN + Target only)'}"
    )
    ...
    train_dataset = CrossDomainDataset(
        ...
        target_inter_path  = paths['inters']['target_train'],
    ...
    valid_dataset = CrossDomainDataset(
        # source_train is used as history context signal for valid/test
        source_inter_path       = paths['inters']['source_train'],
        target_inter_path       = paths['inters']['target_valid'],
    ...
    for epoch in range(num_epochs):
        ...
        for batch in progress_bar:
            ...
            raw_target_embs = e2e_model.target_embedding(target_item_ids)
        ...
        if (epoch + 1) % tr['validation_freq'] == 0:
            ...
            with torch.no_grad():
                # Normalize all target embeddings once
                ...
                for batch in tqdm(valid_loader, desc="Validation"):
                    ...
                    top_k_indices = diffusion_model.sample(
                        ...
                        watched_ids       = target_seq,  # Suppress interacted items
                        w                 = val['cfg_w'],
                        k                 = val['top_k']
                    )

                    batch_hr, batch_ndcg = calculate_metrics(
                        top_k_indices, target_item_ids, k=val['top_k']
                    )
